...
<div class="wrapper">
	...
	<div id="popup">
		<h3 style="text-align:center;">사원 상세 정보</h3>	
...
<script type="text/javascript">
	function openPopup(event) {
		// 링크 클릭 시 페이지가 이동되는 기본 동작이 발생하지 않도록 방해함
		event.preventDefault();
		// 사원 정보를 표시할 팝업을 화면에 표시함
		...
		// onreadystatechange는 XHR의 readyState값이 변할 때마다 발생하는 이벤트임
		xhr.onreadystatechange = function() {
			// 콜백함수가 4번 실행되기는 하지만, 실질적인 작업이 구현된 아래의 소스는 
			// 하나의 XHR에 대해서 단 한 번만 실행된다.
			if (xhr.readyState == 4 && xhr.status == 200) {
				// 서버가 보낸 응답데이터 조회 <-- XHR의 responseText는 응답데이터를 보관하고 있다.
				var text = xhr.responseText;
				// 응답데이터가 JSON 텍스트라면 JSON.parse(json 표기법의 텍스트) 메소드를 실행해서
		...
		// XHR 초기화, url에는 클릭한 <a> 태그의 href 속성에서 조회한 jsp경로가 들어있다.
		xhr.open("GET", url);
	...
	function getEmployees() {
		...
		// * XHR객체의 onreadystatechange 이벤트에, 이벤트 발생 시 실행될 함수를 등록한다.
		// * XHR객체의 readyState 값이 4면 응답을 받은 것이다.
		// * XHR객체의 status 값이 200이면 성공적인 응답을 받은 것이다.
		...
		//   조회된 값을 이용해서 HTML컨텐츠를 만든 다음, 특정 태그에 부착시켜서 화면에 표시되게 한다.
		xhr.onreadystatechange = function () {
			if (xhr.readyState == 4 && xhr.status == 200) {
				var text = xhr.responseText;
