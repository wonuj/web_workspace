	...
	%>
		<div>
			...
			<table class="table">
				...
				%>
					<tr>
						<td><a href="detail.jsp?empid=<%=e.getId() %>&pages=<%=pageNo%>"><%=e.getId() %></a></td>
						<td><a href="detail.jsp?empid=<%=e.getId() %>&pages=<%=pageNo%>"><%=e.getFirstName() %> <%=e.getLastName() %></a></td>
						<td><%=e.getJobId() %></td>
		...
		<%
			...
			// 5. 요청한 페이지가 속한 블록의 시작페이지 번호와 끝페이지 번호 계산하기
